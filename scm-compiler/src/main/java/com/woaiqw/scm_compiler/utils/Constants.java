package com.woaiqw.scm_compiler.utils;

/**
 * Created by haoran on 2018/8/16.
 */

public class Constants {

    // Tag
    public static final String SCM = "ServerClientManagerAnnotation";
    // Log
    static final String PREFIX_OF_LOGGER = "SCMLogger";
    // Generate
    public static final String PACKAGE_OF_GENERATE_FILE = "com.woaiqw.scm_api";
    public static final String ANNOTATION_ACTION_PATH = "com.woaiqw.scm_annotation.annotation.Action";
    public static final String FILE_COMMENT = "\"/** Generated code from ServerClientManager . Do not modify!/\"";
    //module - name
    public static final String KEY_MODULE_NAME = "KEY_MODULE_NAME";
    public static String DEFAULT_MODULE_NAME = "App";
}
